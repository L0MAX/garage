...
//        |regex:/^[(a-zA-Z\s)]+$/u
        return [
            ...
            'displayname' => 'nullable|regex:/^[(a-zA-Z\s)]+$/u|max:50',
            //'dob' => 'required',
            'email' => 'required|email|unique:users,email,'.$this->id,
            'password'=> ($this->id)?'nullable|min:6|max:12|regex:/^(?=.*[a-zA-Z])(?=.*\d).+$/':'required|min:6|max:12|regex:/^(?=.*[a-zA-Z])(?=.*\d).+$/',
    ...
    public function messages()
    {
        return [
            ...
            'join_date.required' => trans('app.Join date is required.'),

            'designation.required' => trans('app.Designation is required.'),
            'designation.regex'  => trans('app.Designation is only alphabets and space.'),
            'designation.max' => trans('app.Designation should not more than 50 character.'),
